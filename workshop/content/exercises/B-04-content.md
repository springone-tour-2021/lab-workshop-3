...
Next, we will set up the maven plugin that allows for both service to contract test execution, and contract to stub (contract embodied as a stand-alone mock service) generation. This gives the necessary `verification` steps that ensure the contract works against the service.
...
Con*fur*guration for the Cat service dictates that we're using JUNIT5 of course, but also we specify a `base class` property. This will get used to transform our contract into the service-to-contract verification test. 
...
When the verification process succeeds, the plugin will generate a maven artifact that can be used by our `cat client` application for running consumer-side tests. Deployment of the artifact is configurable; our application makes use of the local option. We will explore that facet later.
...
Now, begin the initial claws (clause) with `Contract.make`. This method is our point of entry to build the contract, and it represents the top-level object in the contract specification. Then, we can add a description using the `description` method. It's not required, but it facilitates readability.
...
Next we want a response that even a tabby can appreciate. The `response` method is placed adjacent to the `request` object. We can make use of supplemental methods within the `response` object to state what we expect. In this case, it will inspect the HTTP `status` for OK (200), and the `contentType` header should specify a `json` payload. Finally, body inspection is possible using JSON extractions; in this case a `field:value` evaluation occurs.
...
First, `CatRepository` requires mock behaviour, which can be issued within a `@BeforeEach` method. Finally, the mockMVC component gets configured with the Controller under test handling our requests.

Take a look at the BaseClass mock and MVC configuration in context.
...
This step will execute the spring-cloud-contract maven plugin, which generates artifacts and installs them into the local maven repository (`~/.m2` directory).
...
Adding the `spring-cloud-starter-contract-stub-runner` lets us declare tests bound to a specific stub generated 
...
So this is what the client expects in return for a request—hopefully it's the same as on the server—and if it's not, then we will find out during tests. Now, let's take a look at the client itself. It is a simple `restTemplate` consumer that makes the HTTP callout to `cat service` at "/cats/{some cat name}".
...
From here, we can understand what our test will need to do: validate that the call succeeds and that the data comes back without missing bits or failure.
...
Remember the stub runner locates and executes our stub, which is really a [WireMock](http://wiremock.org/) service that models our service. It's not the real service, just a static stub of it.
The test simply calls the `cat-service` REST endpoint provided by the stub and evaluates that the result is consistent with the client's expectations.
...
With the client out of the way, we have finished building tests into our code. Given the success of these tests, we can be confident these cats will land on their feet!