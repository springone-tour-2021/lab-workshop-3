...
yq eval '.data."log.level" = "debug"' \
     <(kubectl get cm argocd-image-updater-config -o yaml -n argocd) \
     | kubectl apply -f -
```
...
```execute-1
kubectl apply -n argocd -f "https://raw.githubusercontent.com/booternetes-III-springonetour-july-2021/cat-service-release-ops/main/tooling/argocd-image-updater-config/argocd-rbac-cm.yaml"
```
